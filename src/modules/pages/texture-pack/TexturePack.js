    ...
    <div className={classNames(styles["mainTexturePackContainer"])}>
      <div className={classNames(styles["blockText"])}>
        ...
        <div className={classNames(styles["texturePackInformation"])}>
          <h4 className={classNames(styles["informationH4"])}>
            Для использования набора ресурсов необходимо установить модификацию Optifine или Sodium с дополнениями.
          </h4>
          <h4 className={classNames(styles["informationH4"])}>
            Без модов он работать не будет.
            <Link to={"/mods"} className={classNames(styles["linkModsPage"])}>
              {" "}
              Моды &#10148;
            </Link>
          </h4>
          <h4 className={classNames(styles["informationH4"])}>
            По вопросам обращаться к игрокам:
        ...
        <div className={classNames(styles["downloadWrapper"])}>
          <a
            href="./texturePack"
            className={classNames(styles["downloadFiles"])}
            download="/static/download/Totem-Elytra-Body-1.19.zip"
          >
            Скачать текстур пак &#10148;
          </a>
        </div>
        <div className={classNames(styles["warnWrapper"])}>
          <Warn inf="Информация, размещенная на сайте, представлена исключительно для удобства просмотра и созерцания прекрасного, и не несет в себе цель распространять интеллектуальную собственность под именем GMGame." />
        </div>
      </div>
        ...
        <div className={classNames(styles["previewPack"])}>
          ...
          <div className={classNames(styles["card_all"])}>
            {arrayTexturePackAll[idActive].items.map((item, i) => {
              return !item.variability_name.find((c) =>
                    ...
                    {item.variability_name.map((el, index) => (
                      <span
                        ...
                        className={classNames(styles["name"], {
                          [styles["copy"]]: copiedIndex === index && i === idShow,
                        })}
      ...
      >
        <div className={classNames(styles["modal_wrapper_card"])}>
          <button className={classNames(styles["close"])} onClick={closeModal}>
            &#128500;
          </button>
          {arrayTexturePackAll[idActive].items.map((item, i) => {
            return idShow !== i ? null : (
              <div className={classNames(styles["view_card_full"])} key={i}>
                ...
                <div className={classNames(styles["text_wrapper"])}>
                  <h2 className={classNames(styles["label"])}>
                    Для того чтобы изменить внешний вид своего предмета, просто переименуйте предмет на наковальне,
                    выбрав один из предложенных вариантов ниже.
                  </h2>
