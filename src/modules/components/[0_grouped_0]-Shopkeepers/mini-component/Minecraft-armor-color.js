...
const MinecraftArmorColor = ({ item }) => {
  ...
  return (
    <p className={classNames(styles["color_style_box"])}>
      - Цвет:
      <span className={classNames(styles["span_color"])}>
        {rgbToHex(item.leather_color?.red, item.leather_color?.green, item.leather_color?.blue)}
      </span>
    </p>
  );
};
