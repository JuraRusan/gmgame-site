...
const ShopUser = () => {
  ...
  return (
    <div className={classNames(styles["shop_wrapper"])}>
      {userShopData[0].owner === "none" ? null : (
        <div className={classNames(styles["user_info"])}>
          <h4 className={classNames(styles["text_line"])}>
            Последнее посещение игрового сервера:
            <span>
              {convertUnixTime(unixTime, "seen")} ( <UnixTimeConverter unixTime={unixTime} /> )
            </span>
          </h4>
          <h4 className={classNames(styles["text_line"])}>
            Всего магазинов:
          ...
          {userShopData[0].villager.length === 0 ? null : (
            <h4 className={classNames(styles["text_line"])}>
              Оставшееся время к удалению:
              <span>
                {convertUnixTime(unixTimeDrop, "drop")} ( <UnixTimeConverter unixTime={unixTimeDrop} /> )
              </span>
            </h4>
          )}
