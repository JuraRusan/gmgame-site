      ...
      <div className={classNames(styles["content"])}>
        {data.user.status === 1 ? (
          <div className={classNames(styles["wrapperText"])}>
            <h4 className={classNames(styles["text"])}>
              Ваша заявка на рассмотрении. Пожалуйста ожидайте.
            </h4>
          </div>
        ) : undefined}
        ...
        {data.user.status === 2 ? (
          <div className={classNames(styles["wrapperText"])}>
            ...
            <h4 className={classNames(styles["text"])}>
              Пожалуйста, ознакомьтесь с правилами и разделом часто задаваемые
              вопросы для получения полезной информации.
            </h4>
            <div className={classNames(styles["version"])}>
              <span className={classNames(styles["versionNumber"])}>
                Java Edition {data.version}
              </span>
            </div>
            <div className={classNames(styles["ipList"])}>
              <label className={classNames(styles["label"])}>
                Серверные адреса для подключения:
              </label>
              {ipArray.map((el, index) => (
                ...
                >
                  <label
                    className={classNames(styles["ip"], {
                      [styles["ipTo"]]: copiedIndex === index,
                    })}
                  >
                    {el}
                  </label>
                  <div className={classNames(styles["copy"])}>
                    {copiedIndex === index ? (
                      <span
                        className={classNames(styles["ico"], styles["icoTo"])}
                      >
                        <CheckSvgComponent
                          width="100%"
                          height="100%"
